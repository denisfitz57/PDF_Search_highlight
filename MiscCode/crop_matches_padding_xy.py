...
# 4) SEARCH_TERM:    what string to search for (case‐insensitive)
# 5) PADDING_X:      extra margin (in PDF points) on left & right
# 6) PADDING_Y:      extra margin (in PDF points) on top & bottom
# 7) MIN_WIDTH:      minimum width (in PDF points) of every crop
# 8) MIN_HEIGHT:     minimum height (in PDF points) of every crop
#
# Make sure OUTPUT_FOLDER exists (or let the script create it).
# ─────────────────────────────────────────────────────────────────────────────
...
PADDING_X  = 10   # horizontal padding (points)
PADDING_Y  = 50    # vertical padding (points)
MIN_WIDTH  = 250   # minimum crop width (points)
MIN_HEIGHT = 600   # minimum crop height (points)
# ─────────────────────────────────────────────────────────────────────────────
...
def expand_rect_to_minimum(rect, min_w, min_h, page_width, page_height):
    """
    Ensure 'rect' is at least (min_w × min_h). Expand evenly if needed, then clamp to page.
    ...
    # Expand horizontally if too narrow
    if curr_w < min_w:
        extra = (min_w - curr_w) / 2.0
        x0 -= extra
        x1 += extra

    # Expand vertically if too short
    if curr_h < min_h:
...
def main():
    ...
    # 3) Counter for (date, page_number) to handle multiple hits
    counter = {}
    ...
    for idx, row in matches.iterrows():
        ...
        page = doc.load_page(0)  # single‐page PDF, so index=0

        # 3.c) Compute padded rectangle with separate PADDING_X, PADDING_Y
        x0 = bbx0 - PADDING_X
        y0 = bby0 - PADDING_Y
        x1 = bbx1 + PADDING_X
        y1 = bby1 + PADDING_Y

        # Clamp this padded rectangle to the page bounds
        x0 = max(x0, 0)
        y0 = max(y0, 0)
        ...
        # 3.e) Render that rectangle to a Pixmap (image)
        pix = page.get_pixmap(clip=final_rect)

        # 3.f) Build output filename:
        #     {SEARCH_TERM}_{date}_p{page_number}_{count:02d}.png
