...
def search_and_highlight(search_term, base_folder, output_directory='.', similarity_threshold=80, 
                         negation_term=None, negation_distance=100):
    """
    ...
    negation_term (str): Term that negates the search if found nearby
    ...
    if negation_term:
        print(f"Will exclude results where '{negation_term}' is within {negation_distance} units")
    ...
    # Apply negation filtering if negation term is provided
    if negation_term and len(search_results) > 0:
        print(f"Checking for negation term '{negation_term}' near matches...")
        
        # Find occurrences of the negation term
        if 'text' in df.columns:
            negation_matches = df[df['text'].str.contains(negation_term, case=False, na=False)].copy()
            
            if len(negation_matches) > 0:
                print(f"Found {len(negation_matches)} occurrences of negation term")
                ...
                for idx, row in search_results.iterrows():
                    ...
                    file_negations = negation_matches[negation_matches['filename'] == filename]
                    ...
                    for neg_idx, neg_row in file_negations.iterrows():
                        ...
                        if distance <= negation_distance:
                            has_nearby_negation = True
                            print(f"Excluding match in {filename} due to nearby negation term (distance: {distance:.2f})")
                            break
    ...
    if negation_term:
        search_term_filename += f"_not_{negation_term.replace(' ', '_')}"
    
    search_results_csv = f"search_results_{search_term_filename}.csv"
    output_pdf_path = os.path.join(output_directory, f"highlighted_{search_term_filename}2.pdf")
    ...
    try:
        ...
        if processed_count > 0:
            ...
            if negation_term:
                result_message += f"\nExcluded instances where '{negation_term}' appears within {negation_distance} units"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--negation', help='Term that negates the match if found nearby', default=None)
    ...
    search_and_highlight(args.search_term, args.base_folder, args.output_dir, 
                         args.threshold, args.negation, args.negation_distance)